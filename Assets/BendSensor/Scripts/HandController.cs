...
public class HandController : MonoBehaviour {
  ...
  public bool destroyHands = false;
  ...
  protected AnimationPlayer animationPlayer_ = new AnimationPlayer();
  ...
  */
  ...
  protected virtual HandModel CreateHand(HandModel model) {
  ...
  */
  ...
  public Frame GetFrame() {
    if (enableRecordPlayback && animationPlayer_.state == RecorderState.Playing)
  ...
  */
  public string FinishAndSaveRecording() {
    string path = animationPlayer_.SaveToNewFile();
    animationPlayer_.Play();
    return path;
  }
  ...
  public void Record() {
    animationPlayer_.Record();
  }

  /** Called in Update() to send frames to the animationPlayer. */
  protected void UpdateAnimator(float goal, float previousGoal) {
    if (!enableRecordPlayback)
      return;

		animationPlayer_.speed = 1f;//Mathf.Abs(goal-previousGoal)*100;
    animationPlayer_.loop = playerLoop;

    if (animationPlayer_.state == RecorderState.Recording) {
      animationPlayer_.AddFrame(leap_controller_.Frame());
    }
	else {
	  // KP: mapping input space value (values 0...1) to index in animation
	  // for now, just fingers[0] controls everything. This setting affects 
	  // in which direction we are looking for next animation frame
	  animationPlayer_.SetGoalFrameIndex (goal);
      animationPlayer_.NextFrame();
    }
  }
}
