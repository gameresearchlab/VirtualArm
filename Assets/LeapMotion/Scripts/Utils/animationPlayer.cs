...
public class AnimationPlayer {
  ...
  /** KP: direction of animation playing (forward or backward) */
  protected bool play_forward_ = true;
  ...
  /** KP: index of the final animation frame */
	protected float goal_frame_index_ = 1;
  ...
  /** KP: Set the direction of playing the recording: forward or backward **/
  public void SetPlayForwardFlag(bool pf){
		play_forward_ = pf;
  }

  /** KP: Set the direction of playing the recording: forward or backward **/
  public void SetGoalFrameIndex(float goal){
	float temp_goal_index = goal * (frames_.Count-1);
	if (goal_frame_index_ != temp_goal_index)
		play_forward_ = (goal_frame_index_ < temp_goal_index);
	goal_frame_index_ = temp_goal_index;
  }
  ...
  public Frame NextFrame() {
	...
	if (frames_.Count > 0 && goal_frame_index_ >= 0 && goal_frame_index_ < frames_.Count) {
		// playing forward
		if (play_forward_) {
			// KP: if possible, go to the next frame toward the direction of goal_frame_index
			if (frame_index_ < goal_frame_index_ && frame_index_ >= 0) {
    ...
    if (frames_.Count > 0) {
      ...
      if (frame_index_ < frames_.Count && frame_index_ >= 0) {
		...
		GameObject.Find ("GUI").GetComponent<BendSensorGUI> ().textToDisplay = frames_.Count.ToString();
    return current_frame_;
  }
